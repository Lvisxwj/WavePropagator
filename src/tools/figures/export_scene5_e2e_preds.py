...
OUT = Path('/tmp/smile2_scene5_e2e_preds')
...
ts = load_module('sam_test_sota_scene5', SAM / 'test_sota.py')
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
print('[export] SAM', SAM, 'device', device, flush=True)

scene_path = Path(ts.cfg['test_path']) / 'scene05.npy'
...
full_inputs = dict(
    meas_raw=meas_raw[:1], shift_mask=shift_mask[:1], PhiPhiT=PhiPhiT[:1],
    meas_H=meas_H[:1], mask3d_batch=mask3d_batch[:1], gt_cuda=test_data[:1].cuda(),
)
np.save(OUT / 'gt.npy', test_data[0].detach().cpu().numpy().astype('float32'))
np.save(OUT / 'measurement.npy', meas_raw[0].detach().cpu().numpy().astype('float32'))
np.save(OUT / 'mask.npy', mask3d.detach().cpu().numpy().astype('float32'))
meta = {'scene_index': 5, 'scene_name': 'scene05', 'models': []}
...
allowed = ['gap_net', 'lambda_net', 'tsa_net', 'mst_l', 'cst_l', 'birnat']
...
try:
    ...
    old_cwd = os.getcwd()
    os.chdir(str(E2E))
    os.environ['SMILE_CONFIG'] = str(E2E / 'configs/runtime_friend/progressive_222_step2_estimate_evolve_v2.yaml')

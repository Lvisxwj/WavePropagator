...
# 从当前实验 YAML 读取路径（由 launch_all.sh 设置 SMILE_CONFIG）
_root = Path(__file__).parent
_cfg_path = Path(os.environ.get('SMILE_CONFIG', _root / 'configs' / 'compact.yaml'))
if not _cfg_path.is_absolute():
    _cfg_path = _root / _cfg_path
with open(_cfg_path, 'r', encoding='utf-8') as _f:
    _cfg = yaml.safe_load(_f)

DATA_ROOT  = Path(_cfg.get('data_root', Path(_cfg['train_path']).parent))

"""
spectral_wave_propagator.py — SWP operator and spectral-field backbone

核心：Spectral Wave Propagator (SpectralWavePropagator)
  - 各向异性阻尼波动方程的频域闭式解
  - 处理欠阻尼（cos/sin）和过阻尼（cosh/sinh）两种情况
  - Mask 软门控（方案 A，默认）或 Klein-Gordon Born 修正（方案 D）
  - FBGW 频带引导加权（可选）
  - windowed variant (historical, not used in SMILE²)
  - 噪声感知阻尼（sigma 参数）

U-Net 骨架参照 MST，SpectralWavePropagator 替代 S-MSA。
"""
...
from model.cassi_operators import MaskConditionedGate, MaskKleinGordonCorrection
from model.spatial_content_modulation import SpatialContentModulation, SimpleMix
...
class SpectralWavePropagator(nn.Module):
    ...
    def __init__(self, dim, mask_mode='A', eps=0.1,
                 fbgw_mode='none',
                 use_spatial_content_modulation=True, use_perchannel=True,
        ...
        self.use_spatial_content_modulation = use_spatial_content_modulation
        self.use_perchannel = use_perchannel
        self.use_spectral_wave = bool(use_spectral_wave)
        self.use_mask_gate = bool(use_mask_gate)
        self.swp_variant = swp_variant
        self.wave_param_mode = str(wave_param_mode).lower()
        self.wave_basis_count = int(wave_basis_count)
        if swp_variant not in ('full', 'legacy', 'dual'):
            raise ValueError(f"swp_variant must be full/legacy/dual, got {swp_variant!r}")
        ...
        if mask_mode == 'A':
            self.mask_op = MaskConditionedGate(dim, eps=eps)
        elif mask_mode == 'D':
            self.mask_op = MaskKleinGordonCorrection(dim, eps=eps)
        else:
            raise ValueError(f"mask_mode 必须是 'A' 或 'D'，得到 '{mask_mode}'")

        # 输出门控
        if use_spatial_content_modulation:
    ...
    def _global_forward(self, x, mask_spatial, sigma=None):
        ...
        if self.mask_mode == 'A':
            if self.use_mask_gate:
                ...
                # Ablation: remove MaskConditionedGate's mask-conditioned amplitude gate
        ...
        if self.use_spatial_content_modulation:
...
# ──────────────────────────────────────────────
# SWPBlock = LN + SWP + residual + LN + post block + residual
# ──────────────────────────────────────────────

class SWPBlock(nn.Module):
    def __init__(self, dim, mask_mode='A',
        ...
        self.wpo   = SpectralWavePropagator(dim, mask_mode=mask_mode, fbgw_mode=fbgw_mode,
                           use_spatial_content_modulation=use_spatial_content_modulation, use_perchannel=use_perchannel,
        ...
        if post_block in ('spatial_content_modulation', 'cmb'):
            self.norm2 = LayerNorm2d(dim)
            self.post  = SpatialContentModulation(dim)
        elif post_block == 'ffn':
            self.norm2 = LayerNorm2d(dim)
            ...
            raise ValueError(f"post_block must be spatial_content_modulation/cmb/ffn/simple/none，得到 '{post_block}'")
...
# ──────────────────────────────────────────────
# SpectralFieldBackbone — 主推模型
# ──────────────────────────────────────────────

class SpectralFieldBackbone(nn.Module):
    """
    U-Net 骨架（参照 MST），SpectralWavePropagator Block 替代 S-MSA。
    ...
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2],
                 ...
                 use_mask_gate=True,
                 post_block='cmb', ffn_mult=2,
                 swp_variant='full', gradient_checkpointing=False,
                 wave_param_mode='free', wave_basis_count=3):
        super().__init__()
        self.dim   = dim
        self.stage = stage
...
class SpectralFieldBackboneKG(SpectralFieldBackbone):
    """SpectralFieldBackbone(use_kg=True) 的别名"""
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2],
                 mask_mode='A',
                 fbgw_mode='none',
                 use_spatial_content_modulation=True, use_perchannel=True,

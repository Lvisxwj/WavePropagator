"""
CASSI mask and coded-measurement operators.

MaskConditionedGate         — 方案 A：初始振幅软门控（默认，推荐）
MaskKleinGordonCorrection  — 方案 D：Klein-Gordon 质量场 + Born 一阶修正
"""
...
class MaskKleinGordonCorrection(nn.Module):
    """
    方案 D：Klein-Gordon 质量场 + Born 一阶修正。

    零阶解 = 普通 WPO（与方案 A 相同）
    一阶修正：
        m_sq = m0_sq.clamp(min=0) * (1 - mask_spatial)
        source = -m_sq * u0_out
        correction = IFFT( FFT(source) * sinc_term * decay )
        out = u0_out + kg_weight * correction

    sinc_term, decay 由 SpectralWavePropagator 计算后传入 apply_correction()。
    """
    ...
    def forward(self, x, mask_spatial):
        ...
        u0 = self.phi(x) * gate
        v0 = self.psi(x) * gate
        m_sq = self.m0_sq.clamp(min=0.0) * (1.0 - mask_spatial)
        return u0, v0, m_sq

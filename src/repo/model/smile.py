...
The backbone remains a SWP-based spectral-field network. Optional physics-fused mode combines the
...
from model.spectral_wave_propagator import SpectralFieldBackbone
from model.spectral_field_components import MaskConditionedInitialField, WavelengthAxisReconstruction
...
class SMILE2(nn.Module):
    def __init__(
        self,
        dim=28,
        unet_stage=2,
        num_blocks=(2, 2, 2),
        use_spatial_content_modulation=True,
        use_perchannel=True,
        ...
        wave_basis_count=3,
        num_field_outputs=1,
        share_estimator_evolver_weights=True,
        return_intermediate_fields=False,
        field_process_mode="plain",
        disable_spectral_field_evolver=False,
        use_mask_gate=True,
    ):
        super().__init__()
        ...
        self.num_field_outputs = int(num_field_outputs)
        self.share_estimator_evolver_weights = bool(share_estimator_evolver_weights)
        self.return_intermediate_fields = bool(return_intermediate_fields)
        self.field_process_mode = str(field_process_mode).lower()
        self.disable_spectral_field_evolver = bool(disable_spectral_field_evolver)
        if self.num_field_outputs < 1:
            raise ValueError("num_field_outputs must be >= 1")
        if self.input_adapter_name not in ("none", "mask", "wavelength"):
            raise ValueError("input_adapter must be none/mask/wavelength")
        if self.field_process_mode not in ("plain", "estimate_evolve_v2", "gated_estimate_v1"):
            raise ValueError("field_process_mode must be plain, estimate_evolve_v2, or gated_estimate_v1")
        ...
        if self.field_process_mode == "estimate_evolve_v2":
            if self.num_field_outputs < 2:
                raise ValueError("estimate_evolve_v2 requires num_field_outputs>=2")
            self.initial_state_stem = MeasurementAwareInitialStateStem(bands=bands)
        elif self.field_process_mode == "gated_estimate_v1":
            if self.num_field_outputs < 2 or self.share_estimator_evolver_weights:
                raise ValueError("gated_estimate_v1 requires num_field_outputs>=2 and share_estimator_evolver_weights=False")
        ...
        def make_backbone():
            return SpectralFieldBackbone(
                dim=dim,
                stage=unet_stage,
                num_blocks=list(num_blocks),
                fbgw_mode="none",
                use_spatial_content_modulation=use_spatial_content_modulation,
                use_perchannel=use_perchannel,
                use_spectral_wave=use_spectral_wave,
                use_mask_gate=use_mask_gate,
                post_block=post_block,
                ffn_mult=ffn_mult,
                swp_variant=swp_variant,
                gradient_checkpointing=gradient_checkpointing,
                wave_param_mode=wave_param_mode,
                wave_basis_count=wave_basis_count,
            )

        if self.share_estimator_evolver_weights:
            self.backbone = make_backbone()
            self.backbones = None
        else:
            self.backbone = None
            self.backbones = nn.ModuleList([make_backbone() for _ in range(self.num_field_outputs)])
    ...
    def forward(self, y, mask, shifted_mask=None, ppt=None):
        ...
        if self.disable_spectral_field_evolver:
            return [x0] if self.return_intermediate_fields else x0

        pred = x0
        outputs = []
        for step_idx in range(self.num_field_outputs):
            if self.share_estimator_evolver_weights:
                pred = self.backbone(pred, shifted_mask)
            else:
                next_pred = self.backbones[step_idx](pred, shifted_mask)
                if step_idx == 0 and self.field_process_mode == "gated_estimate_v1":
        ...
        if self.output_dc:
            ...
            outputs[-1] = pred
        if self.return_intermediate_fields or self.num_field_outputs > 1:
            return outputs
        return pred

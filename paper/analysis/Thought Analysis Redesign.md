...
1. [之前的方向为什么错了](#2-之前的方向为什么错了)
1. [回到问题本质：3D-WPO 到底做了什么](#3-回到问题本质3d-wpo-到底做了什么)
1. [3D-WPO 对 HSI 重建的真实瓶颈](#4-3d-wpo-对-hsi-重建的真实瓶颈)
1. [SOTA 方法的退化处理范式](#5-sota-方法的退化处理范式)
1. [新范式设计：净化-传播-精化](#6-新范式设计净化-传播-精化)
1. [具体模块筛选与评估](#7-具体模块筛选与评估)
1. [降维与参数效率](#8-降维与参数效率)
1. [最终推荐架构](#9-最终推荐架构)
1. [实验路线图](#10-实验路线图)
...
1. **Shift 错位**：不同波段在空间上偏移了 2×(band_index) 像素
1. **Compression 混叠**：28 个波段叠加成 1 层，光谱信息严重混合
1. **传感器噪声**：$\Phi^T n$ 放大后的噪声
...
1. **频率均匀保留**：振荡式传播不会像热方程那样抑制高频
1. **物理结构先验**：色散关系 $\omega_d^2 = v_s^2|\boldsymbol{\omega}*{xy}|^2 + v*\lambda^2\omega_\lambda^2 - (\alpha/2)^2$ 编码了空间-光谱的传播速度差异
...
1. 区分信号和噪声（WPO 做不到）
1. 利用图像先验（纹理、边缘、光谱相关性）（WPO 部分做到——光谱耦合）
1. 自适应不同退化程度（WPO 完全做不到——$\alpha, v_s, t$ 是全局标量）
...
1. **退化权重估计**：一个 1×1 Conv + Sigmoid，从 $\Phi^*$ 和原始 $\Phi$ 的差异中学习逐像素退化权重
...
加入频带引导加权：
$$\hat{u}*\text{out}(\omega) = W*\text{FBGW}(\omega) \cdot K(\omega, t) \cdot \hat{u}_0(\omega)$$
...
> 1. **物理全局传播**（3D-WPO/KG）：在干净初始场上做波动方程闭式解传播
> 1. **局部细节精化**（DWConv FFN）：补充 WPO 无法处理的局部纹理
> 

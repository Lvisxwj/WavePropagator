...
\begin{abstract}
Coded aperture snapshot spectral imaging (CASSI) reconstructs a three-dimensional hyperspectral image from a single two-dimensional coded measurement, leading to a severely under-determined spatial--spectral inverse problem.
Existing end-to-end CNN and Transformer models have substantially improved reconstruction quality, yet their core interaction mechanisms are often implemented as implicit token mixing or generic feature transformations, making it difficult to describe how information propagates over the spatial--spectral field.
This paper proposes \smiletwo{}, a single-pass spectral-field estimation--evolution framework that models CASSI reconstruction from a unified spatial and spectral perspective.
At its core, the Spectral Wave Propagator (\swp{}) adapts a three-dimensional damped wave equation to the discretely sampled \(x,y,\lambda\) hyperspectral field and performs global propagation through a Fourier-domain closed-form solution with \(O(N\log N)\) operator-level complexity.
Around \swp{}, \smiletwo{} first constructs a shift-back field \(H_0\) and a residual-adjoint cue \(Q_0\) through CASSI preparation, then estimates an initial spectral field \(U_1\) with \sfestimator{}, and finally evolves it with \sfevolver{} to produce \(\hat X\).
On the TSA simulation benchmark and real CASSI measurements, \smiletwo{} achieves state-of-the-art end-to-end reconstruction quality with compact computation.
In particular, \smiletwo-L reaches 37.07 dB PSNR and 0.965 SSIM with 3.19M parameters and 38.60G FLOPs, while reducing the spectral angle mapper (SAM) to 0.086 rad among methods evaluated under the same released-weight protocol.
\end{abstract}
...
The mask, dispersion, adjoint shift-back, and normalization map expose how the 2D measurement is entangled with the 3D spectral cube.
...
Model-driven unfolding networks further demonstrate that repeatedly alternating data-consistency updates and learned priors can be highly effective for SCI reconstruction \cite{cai2022dauhst,zhang2024dpu,zhang2024ssr}.
Nevertheless, the central operator studied in this paper is a global Fourier-domain physical propagator.
If such an operator is inserted into a multi-iteration unfolding pipeline, its cost is multiplied by the number of iterations, and its effect is intertwined with the optimization schedule, data-consistency step size, and stage-wise parameter sharing.
This makes it difficult to determine whether the gain comes from the physical propagator itself or from repeated optimization.
We therefore study the single-pass end-to-end regime: the network uses forward-model cues once to prepare an aliased initial field, then performs spectral-field estimation and physical evolution in one forward pass.
This formulation isolates structured spectral-field evolution as a compact reconstruction prior and avoids turning the proposed wave propagator into another component of a heavy iterative solver.

From the CASSI model, the shift-back field \(H_0=\Phi^\top Y\) already contains major scene structures, but it is also contaminated by mask modulation, spectral overlap, and dispersion-induced aliasing.
Thus, a single-pass network should not simply hand this aliased field to a generic token mixer.
Instead, it should first estimate a cleaner spectral field from measurement-consistent cues and then evolve that field with a structured spatial--spectral operator.
...
These works show that the sensing operator, shifted mask, and adjoint shift-back process provide useful initialization and correction cues.
\smiletwo{} follows this insight but does not perform repeated optimization.
Instead, it compresses the forward-model information into CASSI preparation and uses the prepared field as the input to a single-pass spectral-field estimation--evolution network.
This design keeps the computation end-to-end and single-pass, while still retaining measurement-aware physical cues.
...
The shifted sensing mask aligned with the spectral field is denoted by \(\Phi_s\), and the normalization map is \(P=\Phi\Phi^\top\).
...
We first construct a shift-back field:
\begin{equation}
H_0=\Phi^\top(Y/(P+\epsilon)).
\label{eq:h0}
\end{equation}
...
This does not claim that real spectral reflectance is periodic along the physical wavelength axis.
Rather, it provides a learnable frequency basis over the sampled HSI grid.
Consequently, \(\alpha(f_\lambda)\) and \(v_s(f_\lambda)\) are described as spectral-mode-dependent parameters rather than wavelength-specific physical coefficients.
...
\label{eq:pipeline}
\end{equation}
Based on different model widths and parameter sharing choices, we instantiate three compact variants, SMILE-S/M/L, to evaluate the estimation--evolution design under different computational budgets.
...
All training and evaluation experiments are conducted on NVIDIA GPU server GPUs with 80GB memory.
...
\end{equation}
No SAM loss, SSIM loss, spectral-angle loss, or spectral-derivative loss is used.
Except for CASSI preparation, no additional explicit \(\|\Phi\hat X-Y\|\) data-fidelity loss is applied.

\subsection{Quantitative Comparison}

Table~\ref{tab:sota_main} reports per-scene results on the TSA simulation benchmark.
For methods with released weights or reproducible official implementations, we evaluate them under the same protocol.
For methods without released weights or reconstructed outputs, PSNR/SSIM are taken from the corresponding papers and SAM is denoted by ``--''.
\smiletwo-M uses 2.00M parameters and 28.28G FLOPs, reaching 36.57 dB PSNR, 0.961 SSIM, and 0.097 rad SAM.
\smiletwo-L further improves the average score to 37.07 dB PSNR, 0.965 SSIM, and 0.086 rad SAM with 3.19M parameters and 38.60G FLOPs.
These results show that \smiletwo{} achieves state-of-the-art reconstruction quality among end-to-end CASSI methods with compact computation, while also providing favorable spectral-angle fidelity.

\begin{table*}[!t]
\centering
\caption{Quantitative comparison on the TSA simulation benchmark. Each scene cell reports PSNR, SSIM, and SAM(rad) from top to bottom. SAM is unavailable for methods whose released weights or reconstructed outputs are not available.}
\label{tab:sota_main}
\scriptsize
...
The ablation results verify the necessity of estimation--evolution decomposition.
...
This supports the interpretation that \smiletwo{} forms structured cross-band propagation rather than merely using PDE terminology as a visual metaphor.

...
\begin{abstract}
Coded aperture snapshot spectral imaging (CASSI) reconstructs a three-dimensional hyperspectral image from a single two-dimensional coded measurement, resulting in a severely underdetermined spatial--spectral inverse problem.
Recent Transformer-based methods have achieved remarkable progress in end-to-end CASSI reconstruction.
However, attention can learn which spectral bands and spatial regions should interact, but offers little explicit account of how information should propagate to disentangle coded spectral overlap and recover a coherent hyperspectral field.
Moreover, global pairwise interaction incurs quadratic computational complexity.
To address these limitations, we propose \smiletwo{}, a single-pass framework for Spectrally Modeled CASSI Reconstruction via Learned Estimation--Evolution.
Specifically, a CASSI preparation stage constructs a dirty shift-back field, a normalized residual-adjoint field, and shifted-mask information from the forward imaging model.
A \sfestimator{} first uses this information to estimate a cleaner initial spectral field, after which a \sfevolver{} propagates the estimated field into the final hyperspectral reconstruction.
At the core of both components, the Spectral Wave Propagator (\swp{}) adapts a learnable anisotropic three-dimensional damped-wave model to the sampled spatial--spectral field and performs mask-conditioned global interaction through a closed-form Fourier-domain solution.
Unlike implicit pairwise token mixing, \swp{} introduces explicit propagation and damping structures while retaining $O(N\log N)$ operator-level complexity.
Experiments on simulated and real CASSI measurements demonstrate that \smiletwo{} achieves state-of-the-art end-to-end reconstruction quality with compact computation.

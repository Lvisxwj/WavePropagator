...
1. [决策总览](#1-决策总览)
1. [退化估计：不能照搬 DPU，但也不能不做](#2-退化估计不能照搬-dpu但也不能不做)
1. [WPO 内部增强：FBGW 频带引导加权](#3-wpo-内部增强fbgw-频带引导加权)
1. [降维：是否需要，如何做](#4-降维是否需要如何做)
1. [展开框架：GAP vs A-HQS vs ADMM](#5-展开框架gap-vs-a-hqs-vs-admm)
1. [完整架构方案](#6-完整架构方案)
1. [参数量预算与对比](#7-参数量预算与对比)
1. [实验计划](#8-实验计划)
...
1. **噪声水平 $\sigma$**：作为条件信息传给 prior network，让 denoiser 根据当前噪声水平动态调整去噪强度。
...
|deg_weight  |**WPO 输入** |$u_0 = \text{deg_weight} \cdot z + z$（净化后的初始场）                         |
...
1. 显式 PCA 需要在训练前做特征分析，破坏端到端训练
1. 没有 CASSI SOTA 方法用 PCA——说明这不是瓶颈
1. 如果需要参数效率，用**共享权重**（DERNN-LNLT 已验证，参数降为 1/K）比 PCA 更有效
...
1. **改动极小**——只在 GAP 的基础上加一行动量外推：
   ...
   f = f_accel
   ```
1. **不需要引入 Lagrange 乘子**——比 ADMM 简单得多，但理论收敛速率相同（$O(1/K^2)$）。
1. **$\beta_k$ 的选择**：CA²UN 和 Phy-CoSF 都让 $\beta_k$ 可学习（每 stage 一个标量），参数增加 K 个 float32。

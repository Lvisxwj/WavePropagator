...
def gen_meas(gt, mask3d, input_setting='H'):
    ...
    nC = gt.shape[1]
    # CASSI 前向过程
    masked = mask3d * gt                                  # [B, nC, H, W]
    shifted = shift(masked, step=2)                       # [B, nC, H, W+(nC-1)*2]
    meas = torch.sum(shifted, dim=1)                      # [B, H, W_shifted]

    if input_setting == 'Y':
        return meas
    meas_norm = meas / nC * 2
    H_out = shift_back(meas_norm, step=2, nC=nC)          # [B, nC, H, W]
    if input_setting == 'HM':
        return H_out * mask3d
    return H_out  # 'H'
...
def load_mask(mask_path, nC=28):
    ...
    mask_path: 字符串，指向 mask.npy 或 mask.mat 或包含这两者的目录
    """
    path = Path(mask_path)
    if path.is_dir():
        npy = path / 'mask.npy'
    ...
    # mask: [H, W]
    mask3d = np.tile(mask[:, :, np.newaxis], (1, 1, nC))   # [H, W, nC]
    mask3d = np.transpose(mask3d, (2, 0, 1))               # [nC, H, W]
...
def prepare_mask(mask3d, batch_size, device='cuda'):
    ...
    返回：
        mask3d_batch  [B, nC, H, W]   — 用于 CASSI 仿真 / WPO 门控
        shift_mask    [B, nC, H, W_shifted]  — 用于 MS_MSA
    """
    nC, H, W = mask3d.shape
...
def load_training(data_path, max_scenes=205, wavelengths=None):
    ...
    优先读 .npy，否则读 .mat（key='img_expand' 或 'img'，÷65536）。

    首次调用：扫描全部数据，在 data_path/ 下生成：
        [data_name]_info.json  — 场景列表和波长范围
        [data_name]_bands.json — 每个 band 的全局均值 + 波长
    再次调用：检测到两个 json 均已存在，跳过扫描，直接加载。

    wavelengths: 波长列表（nm），由调用方从 physics.WAVELENGTHS 传入；
                 用于填充 json 的 wavelengths_nm 字段。
                 若为 None，json 中 wavelengths_nm 写 null。
    """
...
def load_test(test_path, nC=28):
    ...
    for f in files:
        ...
        # 保证 shape [H, W, nC] → [nC, H, W]
        if img.ndim == 3 and img.shape[-1] == nC:
            img = np.transpose(img, (2, 0, 1))
        scenes.append(img)

    data = np.stack(scenes, axis=0)   # [N, nC, H, W]
...
def _splice_four(train_data, crop_half, nC=28):
    """从 4 张图各取 crop_half×crop_half 拼成一张 [nC, crop, crop]"""
    crop = crop_half * 2
    indices = np.random.randint(0, len(train_data), 4)
    patches = []
    for idx in indices:
        img = train_data[idx]      # [H, W, nC]
        h, w, _ = img.shape
...
def shuffle_crop(train_data, batch_size, crop_size=256, augment=True, device='cuda', nC=28):
    ...
    for idx in indices:
        ...
        patch = img[x0:x0+crop_size, y0:y0+crop_size, :]  # [crop, crop, nC]
        t = torch.from_numpy(np.transpose(patch, (2, 0, 1))).float()
        if augment:
            t = _augment(t)
        gt_batch.append(t)

    # 后半：四块拼接（crop_size//2 × 4）

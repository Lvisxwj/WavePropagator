...
class WPO3D(nn.Module):
    ...
    def forward(self, x, mask_spatial):
        ...
        out_fft, sinc_term, decay = self._wave_modulate(
            u0_fft, v0_fft, alpha, vs, vl, t, C, H, W, x.device
        )

        # ── 方案 B：叠加源项 ──
        if self.mask_mode == 'B' and source is not None:
            src_fft = torch.fft.rfftn(source, dim=(-3, -2, -1))
        ...
        if self.mask_mode == 'D' and m_sq is not None:
            out = self.mask_op.apply_correction(out, m_sq, sinc_term, decay, C, H, W)
    ...
    def __init__(self, dim=28, stage=2, num_blocks=[2, 2, 2],
        ...
        for i in range(stage):
            blocks = nn.ModuleList([
                WPO3DBlock(dim_stage, mask_mode) for _ in range(num_blocks[i])
            ])
        ...
        self.bottleneck = nn.ModuleList([
            WPO3DBlock(dim_stage, mask_mode) for _ in range(num_blocks[-1])
        ])

...
# ──────────────────────────────────────────────
# WaveMST_ML_Unfold — ML-WPO 混合的 unfolding 版
# ──────────────────────────────────────────────

class WaveMST_ML_Unfold(nn.Module):
    """ML-WPO 混合模型的 unfolding 版本

    与 WaveMST_3D_Unfold 完全相同的 unfolding 循环，
    prior network 从 WaveMST_3D 换成 WaveMST_ML。
    不支持源注入（stage4 专注 ML 层消融）。
    """
    ...
    def __init__(self, dim=28, stage=2, num_blocks=None,
        ...
        # Prior: WaveMST_ML（mask_mode 由 use_kg 内部决定：A or D）

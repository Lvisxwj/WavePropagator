"""
train.py — WaveMST 训练入口（stage4: ML-WPO 混合架构）

修改 CONFIG 区域的参数，然后直接运行：
    python train.py
"""
...
# ── ML 层选择 ──
ML_list = ['dwconv_ca', 'wssa', 'freq_band']
ML_TYPE      = ML_list[1]    # 'dwconv_ca' / 'wssa' / 'freq_band'

# ── U-Net 骨架选择 ──
UNET_list = ['symmetric', 'asymmetric', 'alternating']
UNET_MODE    = UNET_list[0]   # 'symmetric' / 'asymmetric'

# ── Unfolding 配置（仅 MODEL_INDEX >= 7 生效）──
NUM_STAGES          = 5
SHARE_STAGE_WEIGHTS = True
MULTI_STAGE_LOSS    = True

# ── 色散介质 ──
USE_DISPERSIVE       = False # unfold 级色散修正
USE_DISPERSIVE_BLOCK = True # lock 级色散修正

# ── 时空优化 ──
USE_AMP       = False
CACHE_PHIPHIT = True

# 数据路径（stage4 在 stage3/stage4/，数据在 dataset/）
...
def main():
    ...
    tag = MODELS[MODEL_INDEX][1] + f'_{ML_TYPE}_{UNET_MODE}'
    if IS_UNFOLDING:
